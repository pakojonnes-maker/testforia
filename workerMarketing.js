...
export async function handleMarketingRequests(request, env) {
    ...
    if (method === "GET" && pathname.match(/^\/api\/restaurants\/[^/]+\/campaigns$/)) {
        ...
        try {
            const campaigns = await env.DB.prepare(`
                SELECT 
                    mc.*,
                    (SELECT COUNT(*) FROM marketing_leads WHERE campaign_id = mc.id) as leads_count,
            ...
            const results = (campaigns.results || []).map(c => ({
                ...
                stats: {
                    // For welcome_modal: leads from marketing_leads, for scratch: claims count
                    leads: c.leads_count || c.total_claims || 0,
    ...
    // ============================================
    // CLAIMS ENDPOINTS (Admin)
    // ============================================
    // List Claims for a restaurant (with filters)
    if (method === "GET" && pathname.match(/^\/api\/restaurants\/[^/]+\/claims$/)) {
        const restaurantId = pathname.split('/')[3];
        const url = new URL(request.url);
        const statusFilter = url.searchParams.get('status'); // active, redeemed, expired
        const limit = parseInt(url.searchParams.get('limit') || '50');
        const offset = parseInt(url.searchParams.get('offset') || '0');
        try {
            let whereClause = 'WHERE cc.restaurant_id = ?';
            const bindings = [restaurantId];
            if (statusFilter && ['active', 'redeemed', 'expired'].includes(statusFilter)) {
                whereClause += ' AND cc.status = ?';
                bindings.push(statusFilter);
            }
            const claims = await env.DB.prepare(`
                SELECT 
                    cc.id, cc.status, cc.customer_contact, cc.magic_link_token,
                    cc.created_at, cc.redeemed_at, cc.opened_at, cc.expires_at,
                    cr.name as reward_name, cr.description as reward_description,
                    mc.name as campaign_name, mc.type as campaign_type
                FROM campaign_claims cc
                LEFT JOIN campaign_rewards cr ON cc.reward_id = cr.id
                LEFT JOIN marketing_campaigns mc ON cc.campaign_id = mc.id
                ${whereClause}
            ...
            const counts = await env.DB.prepare(`
                ...
                FROM campaign_claims WHERE restaurant_id = ?
    ...
    // Admin: manually redeem a claim (by claim ID, not token)
    if (method === "POST" && pathname.match(/^\/api\/claims\/[^/]+\/admin-redeem$/)) {
        const claimId = pathname.split('/')[3];
        try {
            const claim = await env.DB.prepare(
                'SELECT id, status, expires_at FROM campaign_claims WHERE id = ?'
            ).bind(claimId).first();
            if (!claim) return createResponse({ success: false, message: 'Claim not found' }, 404);
            if (claim.status === 'redeemed') return createResponse({ success: false, message: 'Already redeemed' }, 400);
            const redeemedAt = new Date().toISOString();
            await env.DB.prepare(
                'UPDATE campaign_claims SET status = ?, redeemed_at = ? WHERE id = ?'
            ).bind('redeemed', redeemedAt, claimId).run();
    ...
    if (method === "GET" && pathname.match(/^\/api\/r\/[a-zA-Z0-9]+$/)) {
        ...
        try {
            ...
            // Check if restaurant requires PIN for redemption
            const pinCheck = await env.DB.prepare(
                'SELECT redeem_pin FROM restaurant_details WHERE restaurant_id = ?'
            ).bind(claim.restaurant_id).first();
            const requiresPin = !!pinCheck?.redeem_pin;
            return createResponse({
                success: true,
                claim: {
                    id: claim.claim_id,
    ...
    if (method === "POST" && redeemMatch) {
        ...
        try {
            ...
            // PIN Validation: check if restaurant has a redeem PIN configured
