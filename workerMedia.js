// workerMedia.js - Versión actualizada para nueva arquitectura de medios
export async function handleMediaRequests(request, env) {
  ...
  if (request.method === "GET" && url.pathname.match(/^\/restaurants\/[\w-]+\/media$/)) {
    try {
      ...
      // Consulta robusta para medios del restaurante incluyendo roles
      ...
      // Obtener traducciones de nombres de platos
      if (mediaQuery.results && mediaQuery.results.length > 0) {
        const dishIds = [...new Set(mediaQuery.results.map(m => m.dish_id))];
        
        for (const dishId of dishIds) {
          const translation = await env.DB.prepare(`
            SELECT value FROM translations 
            WHERE entity_id = ? AND entity_type = 'dish' 
            AND field = 'name' AND language_code = 'es'
            LIMIT 1
          `).bind(dishId).first();
          
          // Actualizar el nombre del plato en los resultados
          if (translation && translation.value) {
            mediaQuery.results.forEach(media => {
              if (media.dish_id === dishId) {
                media.dish_name = translation.value;
              }
            });
          }
        }
      ...
      const media = (mediaQuery.results || []).map(m => {
        ...
        const mediaItem = {
          id: m.id,
          dish_id: m.dish_id,
          dish_name: m.dish_name || 'Plato sin nombre',
        ...
        if (m.media_type === 'video') {
          const thumbnailUrl = findThumbnailUrl(mediaQuery.results, m.dish_id, url.origin);
